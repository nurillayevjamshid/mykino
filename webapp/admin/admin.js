...
// ===== FIFA Jonli (yopiq kanal promo) =====
let fifaLiveUploadedUrl = '';
let fifaLiveSettingsLoaded = false;
let lastSavedFifaLiveEnabled = false;

function setFifaLivePreview(url) {
  const img = document.getElementById('fifaLiveImagePreview');
  const area = document.getElementById('fifaLiveUploadArea');
  if (!img || !area) return;
  if (url) {
    img.src = proxiedPoster(url);
    img.style.display = 'block';
...
async function loadFifaLiveSettings() {
  if (fifaLiveSettingsLoaded) return;
  try {
    const res = await fetch('/api/settings', { cache: 'no-store' });
    const json = await res.json();
    if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
    const f = json.fifaLive || {};
    const enabledEl = document.getElementById('fifaLiveEnabled');
    if (enabledEl) enabledEl.checked = Boolean(f.enabled);
    lastSavedFifaLiveEnabled = Boolean(f.enabled);
    const urlEl = document.getElementById('fifaLiveImageUrl'); if (urlEl) urlEl.value = f.imageUrl || '';
    const chEl = document.getElementById('fifaLiveChannelUrl'); if (chEl) chEl.value = f.channelUrl || '';
    const titleEl = document.getElementById('fifaLiveTitle'); if (titleEl) titleEl.value = f.title || '';
    const subEl = document.getElementById('fifaLiveSubtitle'); if (subEl) subEl.value = f.subtitle || '';
    const btnEl = document.getElementById('fifaLiveButtonText'); if (btnEl) btnEl.value = f.buttonText || '';
    fifaLiveUploadedUrl = f.imageUrl || '';
    setFifaLivePreview(f.imageUrl || '');
    fifaLiveSettingsLoaded = true;
  } catch (err) {
    setFifaLiveStatus(`Yuklashda xato: ${err.message}`, 'error');
  }
}

async function saveFifaLiveSettings(options = {}) {
  const enabledEl = document.getElementById('fifaLiveEnabled');
  const enabled = enabledEl?.checked || false;
  const imageUrl = (document.getElementById('fifaLiveImageUrl')?.value || fifaLiveUploadedUrl || '').trim();
  const channelUrl = (document.getElementById('fifaLiveChannelUrl')?.value || '').trim();
  const title = (document.getElementById('fifaLiveTitle')?.value || '').trim();
  const subtitle = (document.getElementById('fifaLiveSubtitle')?.value || '').trim();
  const buttonText = (document.getElementById('fifaLiveButtonText')?.value || '').trim();
  ...
  try {
    const res = await fetch('/api/settings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fifaLive: { enabled, imageUrl, channelUrl, title, subtitle, buttonText } }),
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);

    const saved = (json && json.fifaLive) || {};
    const chEl = document.getElementById('fifaLiveChannelUrl');
    if (chEl) chEl.value = saved.channelUrl || '';
    lastSavedFifaLiveEnabled = Boolean(saved.enabled);
...
document.getElementById('fifaLiveImageFile')?.addEventListener('change', async (e) => {
  ...
  try {
    const raw = await readFileAsDataUrl(file);
    const dataUrl = await compressImageDataUrl(raw, 1280, 720, 0.85);
    const url = await uploadCategoryImage(dataUrl, 'fifa-live-' + Date.now().toString(36));
    fifaLiveUploadedUrl = url;
    document.getElementById('fifaLiveImageUrl').value = url;
...
document.getElementById('fifaLiveImageUrl')?.addEventListener('input', (e) => {
  setFifaLivePreview(e.target.value.trim());
});

document.getElementById('fifaLiveSaveBtn')?.addEventListener('click', () => saveFifaLiveSettings());
document.getElementById('fifaLiveEnabled')?.addEventListener('change', () => {
  saveFifaLiveSettings({ revertOnFail: true });
});
document.getElementById('fifaLiveClearBtn')?.addEventListener('click', () => {
  if (!confirm('FIFA Jonli maydonlari tozalansinmi?')) return;
  const enabledEl = document.getElementById('fifaLiveEnabled');
  if (enabledEl) enabledEl.checked = false;
  ['fifaLiveImageUrl', 'fifaLiveChannelUrl', 'fifaLiveTitle', 'fifaLiveSubtitle', 'fifaLiveButtonText'].forEach((id) => {
