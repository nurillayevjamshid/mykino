...
from .keyboards import admin_panel, user_menu
from .storage import load_movies, parse_movie_caption, search_movies, upsert_movie
    ...
    movie.update(
        {
            ...
            "sourceUrl": f"https://t.me/{username}/{message.message_id}" if username else "",
        }
    )
    return movie
...
async def start(message: Message, settings: Settings) -> None:
    ...
    is_admin = _is_admin(message.from_user.id if message.from_user else None, settings.admin_ids)
    await message.answer(
        "Assalomu alaykum!\n\n"
        "🎬 Kinolarni mini app orqali tomosha qiling.\n"
        "🔍 Kino nomi yoki kodini yuborsangiz, bot qidirib beradi.",
        reply_markup=user_menu(settings.webapp_url, is_admin),
    )
...
async def admin(message: Message, settings: Settings) -> None:
    ...
    categories = sorted({movie["genre"] for movie in movies})
    await message.answer(
        "<b>Admin panel</b>\n\n"
